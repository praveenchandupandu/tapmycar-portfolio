...
module.exports = async function handler(req, res) {
  ...
  const towName       = String(b.tow_company_name || '').trim().slice(0, 200);
  const towPhone      = String(b.tow_company_phone || '').trim().slice(0, 60);
  const towAddress    = String(b.tow_company_address || '').trim().slice(0, 400);
  const message       = String(b.message || '').trim().slice(0, 1000);

  if (!token || !plate || !towName) {
    return res.status(400).json({ error: 'token, license_plate, and tow_company_name are required' });
  }

  // Per-tag rate limit (a real car can't be towed twice an hour by the same operator)
    ...
    const html =
      '<div style="font-family:Inter,Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 22px;color:#111">' +
        ...
        '<div style="background:#FFF7ED;border:1.5px solid #FED7AA;border-radius:14px;padding:18px;margin-bottom:18px">' +
          ...
          '<div style="font-size:14px;color:#9A3412;line-height:1.6">Hi ' + esc(ownerName) + ',<br>A tow operator scanned your TapMyCar sticker on <b>' + esc(vehicle) + '</b> (plate <b>' + esc(plate) + '</b>) and sent you this notification.</div>' +
        '</div>' +
        '<div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:14px;padding:18px;font-size:14px;line-height:1.6;color:#111">' +
          '<div style="font-size:11px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Tow operator details</div>' +
          '<div><b>Company:</b> ' + esc(towName) + '</div>' +
          phoneRow +
          addrRow +
          msgRow +
        '</div>' +
        '<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;padding:14px;font-size:12px;color:#1E40AF;line-height:1.6;margin-top:18px">' +
          '<b>Heads up:</b> this message came from someone who scanned your sticker. Verify the company and call them directly using the number above before driving to any address. If anything looks wrong, reply to this email or contact <a href="mailto:support@tapmycar.io" style="color:#1E40AF">support</a>.' +
        '</div>' +
        ...
        '<div style="font-size:11px;color:#9CA3AF;text-align:center;margin-top:22px">You\'re receiving this because someone scanned your TapMyCar sticker.</div>' +
      '</div>';
  ...
  // Trigger push notification (best-effort, no await on response)
  try {
    ...
    if (subs && subs.length) {
      const payload = JSON.stringify({
        ...
        body: 'Your car (' + plate + ') was towed. Tap to view details.',
        url: '/my-tows.html'
      });
